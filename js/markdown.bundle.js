...
g.use({
  ...
  renderer: {
    code({ text: text2, lang }) {
      const raw = text2.replace(/\n$/, "");
      const name = normalizeLang(lang);
      let highlighted;
      try {
        if (name && core_default.getLanguage(name)) {
          highlighted = core_default.highlight(raw, { language: name }).value;
        } else {
          highlighted = core_default.highlight(raw, { language: "plaintext" }).value;
        }
      } catch {
        highlighted = escapeHtmlText(raw);
      }
      const safeName = name && core_default.getLanguage(name) ? String(name).replace(/[^\w-]/g, "-").replace(/^-|-$/g, "") || "lang" : "plaintext";
      return `<pre><code class="hljs language-${safeName}">${highlighted}</code></pre>
`;
    }
  }
});
var PURIFY_FORBID_TAGS = [
  "img",
  "svg",
  "picture",
  "iframe",
...
function renderMarkdownToSafeHtml(src) {
  ...
  let html2;
  try {
    html2 = g.parse(String(src), { async: false });
  } catch {
...
function fillMessageBodyMarkdown(bodyEl, text2) {
  bodyEl.classList.add("webui-msg__body--md");
  bodyEl.innerHTML = renderMarkdownToSafeHtml(text2);
}
function fillMessageBodyPlain(bodyEl, text2) {
  bodyEl.classList.remove("webui-msg__body--md");
  bodyEl.textContent = text2;
}
export {
  fillMessageBodyMarkdown,
  fillMessageBodyPlain,
